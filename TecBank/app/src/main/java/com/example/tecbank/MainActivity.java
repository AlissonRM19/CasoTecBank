...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(com.example.tecbank.Socket.sistemaInit == false){
            com.example.tecbank.Socket.sistemaInit = true;
            new Thread(() -> {
                try {
                    com.example.tecbank.Socket.iniciarSocket();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }).start();
    ...
    private void procesarMensaje(){
        ...
        runOnUiThread(() -> {
            if (com.example.tecbank.Socket.message != null) {
                String message = com.example.tecbank.Socket.message;
                if ("1".equals(message)) {
